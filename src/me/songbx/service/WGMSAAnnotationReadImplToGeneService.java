...
public class WGMSAAnnotationReadImplToGeneService {
	private HashMap<String, ArrayList<Gene>> geneHashMapArrayList = new HashMap<String, ArrayList<Gene>>();
	public WGMSAAnnotationReadImplToGeneService(AnnotationReadImpl annotationReadImpl){
		...
		while(chrGeneIt.hasNext()){
			...
			for( int i=1; i< geneHashMapArrayList.get(key).size(); i++ ){
				int newStart = geneHashMapArrayList.get(key).get(i-1).getEnd()+1;
				int newEnd = geneHashMapArrayList.get(key).get(i).getStart()-1;
				Gene gene = new Gene(newStart, newEnd, key);
				String name = ""+newStart+"_"+newEnd;
				gene.setName(name);
				genes.add(gene);
			}
			geneHashMapArrayList.get(key).addAll(genes);
			Collections.sort(geneHashMapArrayList.get(key));
